...
public final class ColorUtils {
  ...
  public static int HSLToColor(@NonNull float[] paramArrayOffloat) {
    ...
    switch (i) {
      ...
      case 0:
        j = Math.round(255.0F * (f2 + f4));
        k = Math.round(255.0F * (f3 + f4));
        m = Math.round(255.0F * f4);
      case 1:
        j = Math.round(255.0F * (f3 + f4));
        k = Math.round(255.0F * (f2 + f4));
        m = Math.round(255.0F * f4);
      case 2:
  ...
  public static int XYZToColor(@FloatRange(from = 0.0D, to = 95.047D) double paramDouble1, @FloatRange(from = 0.0D, to = 100.0D) double paramDouble2, @FloatRange(from = 0.0D, to = 108.883D) double paramDouble3) {
    ...
    if (d1 > 0.0031308D) {
      paramDouble1 = 1.055D * Math.pow(d1, 0.4166666666666667D) - 0.055D;
    } else {
...
/* Location:              /opt/home/jayk-storage/Downloads/quadstellar/classes-dex2jar.jar!/android/support/v4/graphics/ColorUtils.class
